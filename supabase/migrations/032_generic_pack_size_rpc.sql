-- Replaces nearby_prices RPC with a generic pack_size approach.
-- Now supports all pack sizes (single, 4_pack, 6_pack, 8_pack, 10_pack, 12_pack, 24_pack)
-- via SPLIT_PART instead of hard-coded CASE WHEN.
-- Also adds 'multipack' as a filter value (any pack_size != 'single').
...
AS $$
  SELECT
    ...
    CASE
      WHEN prod.pack_size = 'single' THEN p.price
      ELSE ROUND((p.price / NULLIF(SPLIT_PART(prod.pack_size, '_', 1), '')::NUMERIC)::NUMERIC, 2)
  ...
  WHERE s.is_active = true
    ...
    AND (
      ...
      OR (p_pack_size_filter = 'multipack' AND prod.pack_size != 'single')
  ...
  ORDER BY
    CASE WHEN p_sort_by = 'price' THEN
      CASE
        WHEN prod.pack_size = 'single' THEN p.price
        ELSE p.price / NULLIF(SPLIT_PART(prod.pack_size, '_', 1), '')::NUMERIC
      END
    END ASC NULLS LAST,
...
COMMENT ON FUNCTION nearby_prices IS
  ...
  'filtered by variant and pack_size (including ''multipack'' = any non-single pack), '
  'sorted by price/distance/name with distance tiebreaker.';
